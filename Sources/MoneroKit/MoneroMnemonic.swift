...
public enum MoneroMnemonic {
    public static let wordCount = 25
    ...
    public static let prefixLength = 3
    ...
    private static let wordSet = Set(wordList)

    public static func isValid(word: String, partial: Bool = false) -> Bool {
        if partial {
            return wordList.contains { $0.hasPrefix(word) }
        } else {
    ...
    public static func suggestions(prefix: String) -> [String] {
        ...
        return wordList.filter { $0.hasPrefix(prefix) }
    }

    /// Selects the checksum word for `dataWords` (the first 24 words of a phrase): CRC32 over
    /// the concatenated `prefixLength`-letter prefixes picks which data word is repeated last.
    public static func checksumWord(dataWords: [String]) -> String {
        let prefixes = dataWords.map { String($0.prefix(prefixLength)) }.joined()
        let crc = crc32(Array(prefixes.utf8))
        return dataWords[Int(crc % UInt32(dataWords.count))]
    }

    public static func validateChecksum(words: [String]) throws {
        guard words.count == wordCount else {
            throw ValidationError.invalidWordCount
        }

        guard checksumWord(dataWords: Array(words.prefix(wordCount - 1))) == words.last else {
            throw ValidationError.invalidChecksum
        }
    }

    // CRC32 (IEEE 802.3, reflected, init/xorout 0xFFFFFFFF) — the same algorithm as
    // java.util.zip.CRC32 and monero-core's Electrum-words checksum.
